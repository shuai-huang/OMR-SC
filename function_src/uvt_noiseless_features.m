function foo = uvt_noiseless_features(LN_num, num_proj_img, N, N_half, r_cut, l_max, snr_level, density_file, aspire_dir, proj_dir, feature_dir)
    ...
    dlmwrite(strcat(feature_dir, '/spatial_radial_lgwt_feat_noiseless'), radial_feat_est_out, 'delimiter', ' ', 'precision', 12);
    ...
    for (l_out = [ l_max ])
        ...
        if (exist(strcat(feature_dir, '/fourier_ac_decomposition_feat_noiseless_', num2str(l_out))))
            system(['rm ' feature_dir '/fourier_ac_decomposition_feat_noiseless_' num2str(l_out)]);
        end
        ...
        for (l=0:l_out)
            ...
            for (m_idx=1:l_val)
                ...
                dlmwrite(strcat(feature_dir, '/fourier_ac_decomposition_feat_noiseless_', num2str(l_out)), cor_feat_est_tmp, '-append', 'delimiter', ' ', 'precision', 12)
    ...
    dlmwrite(strcat(feature_dir, '/spatial_radial_uniform_feat_noiseless'), radial_feat_est_out, 'delimiter', ' ', 'precision', 12);
