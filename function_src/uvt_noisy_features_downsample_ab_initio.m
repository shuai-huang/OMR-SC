function foo = uvt_noisy_features_downsample_ab_initio(LN_num, num_proj_img, N, N_half, downsample_unit, r_cut, l_max, aspire_dir, proj_dir, proj_downsample_dir, feature_downsample_dir)
    ...
    % aspire_dir:       a string, the direcotry where the aspire package is saved
    ...
    % feature_downsample_dir:      a string, the directory where the features computed from downsampled projections are saved
    ...
    dlmwrite(strcat(feature_downsample_dir, '/spatial_radial_lgwt_feat_noisy'), radial_feat_est_out, 'delimiter', ' ', 'precision', 12);
    ...
    for (l_out = [ l_max ])
        ...
        if (exist(strcat(feature_downsample_dir, '/fourier_ac_decomposition_feat_noisy_', num2str(l_out))))
            system(['rm ' feature_downsample_dir '/fourier_ac_decomposition_feat_noisy_' num2str(l_out)]);
        end
        ...
        for (l=0:l_out)
            ...
            for (m_idx=1:l_val)
                ...
                dlmwrite(strcat(feature_downsample_dir, '/fourier_ac_decomposition_feat_noisy_', num2str(l_out)), cor_feat_est_tmp, '-append', 'delimiter', ' ', 'precision', 12)
    ...
    dlmwrite(strcat(feature_downsample_dir, '/spatial_radial_uniform_feat_noisy'), radial_feat_est_out, 'delimiter', ' ', 'precision', 12);
