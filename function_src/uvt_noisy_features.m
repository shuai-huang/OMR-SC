function foo = uvt_noisy_features(LN_num, num_proj_img, N, N_half, r_cut, l_max, snr_level, density_file, aspire_dir, proj_dir, feature_dir)

    % generate noisy projection images, and estimate the necessary method-of-moments features
    ...
    % l_max:            the largest spherical harmonic degree, by default it is set to 10, (2*l_max+1) should be no greater than the rank of the autocorrelation feature matrix
    % snr_level:        the signal to noise ratio, i.e. the power of signal divided by the power of noise, by default it is set to 0.1
    ...
    rng_num=1;  % random seed
    rng(rng_num)

    load(density_file)

    rots_true = rand_rots(num_proj_img);
    projs = cryo_project(den_3d, rots_true);
    average_signal_norm = sqrt(sum(projs(:,:,1:100).^2,'all')/100); % compute the average norm of projection images based on the first 100 projection images
    sigma = average_signal_norm/N * sqrt(1/snr_level);
    projs = projs + sigma * randn(size(projs));

    system(['mkdir -p ' proj_dir])
    system(['mkdir -p ' feature_dir])

    proj_image_mat = projs;
    save(strcat(proj_dir, '/raw_images.mat'), 'proj_image_mat')
    ...
    psd = cryo_noise_estimation(projs(:,:,1:n));
    %plot(psd(n,:));
    ...
    [ noise_v_r ] = estimate_noise_real(projs(:,:,1:n));
    ...
    %[ denoised_coeff_ccwf , ~ , ~ , num_eigs ] = jobscript_CCWF_cgshrink_jsb( ctfid , w_f_rad , ctfs_rad , basis , sample_points , coeff_mean , coeff_ymu , noise_v_r );
    ...
    dlmwrite(strcat(feature_dir, '/spatial_radial_lgwt'), radial_lgwt_file, 'delimiter', ' ', 'precision', 12) 
    ...
    dlmwrite(strcat(feature_dir, '/spatial_radial_lgwt_feat_noisy'), radial_feat_est_out, 'delimiter', ' ', 'precision', 12);
    ...
    dlmwrite(strcat(feature_dir, '/spatial_radial_lgwt_map'), radial_map, 'delimiter', ' ', 'precision', 12)
    ...
    %% fourier_ac_decomposition_feat_noisy_ contains the transformed spherical harmonic coefficients, i.e. A_l \times O_l, where O_l is unknown orthogonal matrices
    ...
    for (l_out = [ l_max ])
        ...
        if (exist(strcat(feature_dir, '/fourier_ac_decomposition_feat_noisy_', num2str(l_out))))
            system(['rm ' feature_dir '/fourier_ac_decomposition_feat_noisy_' num2str(l_out)]);
        end
        if (exist(strcat(feature_dir, '/l_idx_fourier_ac_decomposition_', num2str(l_out))))
            system(['rm ' feature_dir '/l_idx_fourier_ac_decomposition_' num2str(l_out)]);
        end
        ...
        for (l=0:l_out)
            ...
            for (m_idx=1:l_val)
                ...
                dlmwrite(strcat(feature_dir, '/fourier_ac_decomposition_feat_noisy_', num2str(l_out)), cor_feat_est_tmp, '-append', 'delimiter', ' ', 'precision', 12)
                dlmwrite(strcat(feature_dir, '/l_idx_fourier_ac_decomposition_', num2str(l_out)), radial_idx_tmp, '-append', 'delimiter', ' ', 'precision', 12)
            end
    ...
    dlmwrite(strcat(feature_dir, '/fourier_radial_uniform_map'), fourier_radial_map, 'delimiter', ' ', 'precision', 12)
    ...
    dlmwrite(strcat(feature_dir, '/spatial_radial_uniform_feat_noisy'), radial_feat_est_out, 'delimiter', ' ', 'precision', 12);
